...
def main():
    ...
    companies = get_available_companies(model_dir)
    ...
    # Dropdown menu to select a company
    selected_company = st.selectbox("Select a company:", companies)

    # Construct the model file path dynamically
    model_path = os.path.join(model_dir, f"xgb_model_final_{selected_company}.pkl")
    ...
    if st.button("Get Message"):
        ...
        st.success(f"The prediction for tomorrows is that the stock goes: {predict_next_day_xgboost_api(xgb_model_final, selected_company)}")
